...
export function determineVersion(input) {
    const byteLength = Buffer.byteLength(input, 'utf8');
    for (let idx = 1; idx <= 40; idx++) {
        if (byteLength <= maxInputSizes[idx]) {
            return idx;
        }
    }
    throw new Error('Input too large');
}
export function getTotalDataBits(version) {
    const codewords = totalDataCodewords[version];
    return codewords * 8;
}
